...
import { isCustomerHolder, } from "mrmkmcib-crm";
import { setTrimmedTop as setCustomerDashboardTrimmedTop, setCurrentCustomerDashboardQlikUrl, } from 'mrmkmcib-dashboard';
...
import { supParamNames, HistoryMobileApp, navigationExecutor, EnumsApp, performNavigateBack, performNavigateToEntity, } from 'mrmkmcib-app';
 ...
 */
export const performDashboardExpandedModeToggle = () => (dispatch, getState) => {
    let state = getState();
 ...
 */
export const performInputSearchAffiliateList = (search) => (dispatch) => {
 ...
 */
export const performAffiliateListSearch = () => (dispatch) => {
 ...
 */
export const performSearchAffiliateListEnable = () => (dispatch) => {
    dispatch(actionsCustomer.performSearchAffiliateListEnable());
};
/*
 * Thunk dispatched by "Customer" screen. Thunk chain used to enable search mode.
 */
export const performSearchAffiliateListDisable = () => (dispatch) => {
...
export const performChangeTab = (index, value) => (dispatch, getState) => {
    ...
    if (reducerState.currentTab == Enums.CustomerManagedTab.Main) {
        ...
        dispatch(thunkAgentList.performOpenAgentListScreen(reducerState.currentCustomerManaged.agentList, reducerState.currentCustomerManaged, null, Enums.AgentListContextMode.Customer, agentListAccessLevel));
    }
 ...
 */
export const performPopoverHolderShow = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    ...
    else if (reducerState.currentCustomer.id != '' && reducerState.currentCustomer.holder != null) {
 ...
 */
export const performPopoverHolderHide = () => (dispatch) => {
...
export const performPopoverCuratorShow = () => (dispatch, getState) => {
    ...
    else if (reducerState.currentCustomer.id != '' && reducerState.currentCustomer.curator != null) {
 ...
 * Thunk dispatched by "Customer" screen. Thunk chain used to hide popover.
 */
export const performPopoverCuratorHide = () => (dispatch) => {
    dispatch(actionsCustomer.performPopoverCuratorHide());
};
export const navigateBackHistoryMobileApp = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    ...
    dispatch(HistoryMobileApp.popContent(popContentCustomer ?
                ...
                case EnumsApp.HistoryMobileAppType.Activity: {
                    if (content && content.payload &&
                        ...
                        if (content.payload.activity.data.customer &&
                            ...
                            dispatch(navigateToCustomerScreen(content.payload.activity.data.customer, Enums.CustomerMode.SameType, Enums.ShowCustomer.Show));
                        }
                        let state = getState();
        ...
        dispatch(HistoryMobileApp.popContent(popContentAppCRM ?
            ...
            if (content) {
                switch (content.type) {
                    case EnumsApp.HistoryMobileAppType.None: {
                        ...
                        if (qlikUrl) {
                            dispatch(performNavigateToEntity(Object.assign({}, defaultValues.NavigateToEntity, { navigationType: EnumsApp.NavigationType.AppDashboard, qlikUrl })));
                            return;
                        }
                    }
                    case EnumsApp.HistoryMobileAppType.Customer: {
                        if (content &&
                            ...
                            dispatch(thunkCustomer.callGetCustomerActivityList(Object.assign({}, defaultValues.CustomerManaged, content.payload.customer)));
                        }
                        break;
                    }
                    case EnumsApp.HistoryMobileAppType.Agent: {
                        if (content.payload && content.payload.agent) {
                            dispatch(thunkAgent.performOpenAgentScreen(content.payload.agent.data, Object.assign({}, defaultValues.CustomerManaged, content.payload.agent.customer), Enums.AgentMode.Watch, content.payload.agent.contextMode));
                        }
                        return;
                    }
                }
 ...
 */
export const customerNavigateBack = () => (dispatch, getState) => {
    const state = getState();
    const reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    const navigationHistory = reducerState.customerNavigationHistory;
    const navigationHistoryLength = navigationHistory.length;
    if (navigationHistoryLength > 1) {
        const lastNavigationStep = navigationHistory[navigationHistory.length - 1];
        ...
        if (currentCustomerId) {
            if (reducerState.currentMode != Enums.CustomerMode.SameType) {
                ...
                dispatch(thunkCustomer.callGetCustomerActivityList(Object.assign({}, defaultValues.CustomerManaged, { id: currentCustomerId, organizationType: customerManaged.organizationType })));
            }
            else {
                if (previousCustomerId) {
                    dispatch(thunkCustomer.callGetCustomerActivityList(Object.assign({}, defaultValues.CustomerManaged, { id: previousCustomerId, organizationType: customerManaged.organizationType })));
                }
            }
...
export const navigateToAgentListScreen = () => (dispatch, getState) => {
    ...
    const currentCustomer = reducerState.currentCustomerManaged != null &&
    ...
    dispatch(HistoryMobileApp.pushContent({
        ...
        data: Object.assign({}, defaultValues.HistoryMobileAppData, { customer: reducerState.currentCustomerManaged })
    }));
};
/*
 * Thunk dispatched by "Customer" screen. Thunk chain used to show popover.
 ...
 */
export const navigateToEmployeeViewScreen = () => (dispatch) => {
    dispatch(actionsCustomer.navigateToEmployeeViewScreen());
    dispatch(SplitPanelActions.pushContent(Enums.NavigationContentCustomer.EmployeeView, util.getNavigationContentString(Enums.NavigationContentAppCrm.AppCRM_Customer)));
};
export const performReturnToCustomerScreen = (splitPanelName) => (dispatch, getState) => {
    dispatch(HistoryMobileApp.popContent(splitPanelName))
        ...
        if (content && (content.type == EnumsApp.HistoryMobileAppType.Customer) &&
            ...
            dispatch(thunkCustomer.customer_callGetCustomer(Object.assign({}, defaultValues.CustomerManaged, content.payload.customer)));
            dispatch(thunkCustomer.callGetCustomerActivityList(Object.assign({}, defaultValues.CustomerManaged, content.payload.customer)));
        }
    });
};
export const performAddCustomerToHistoryMobileApp = (index, splitPanelName, customer) => (dispatch, getState) => {
    dispatch(HistoryMobileApp.pushContent({
        splitPanelName,
        index,
        type: EnumsApp.HistoryMobileAppType.Customer,
        data: Object.assign({}, defaultValues.HistoryMobileAppData, { customer }),
    }));
};
/*
 * Thunk dispatched by "Customer" screen. Thunk chain used to navigate to occasion list screen.
 */
export const navigateToOccasionListScreen = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    let occasionListAccessLevel = reducerState.currentCustomerManaged.id != '' ?
    ...
    if (reducerState.currentCustomerManaged.id != '') {
        dispatch(performAddCustomerToHistoryMobileApp(Enums.NavigationContentCustomer.OccasionList, util.getNavigationContentString(Enums.NavigationContentAppCrm.AppCRM_Customer), reducerState.currentCustomerManaged));
        dispatch(thunkOccasionList.performOpenOccasionListScreen(reducerState.currentCustomerManaged.occasionList, { contextMode: Enums.OccasionListContextMode.CustomerManaged,
        ...
        dispatch(navigationExecutor(EnumsApp.NavigationStep.Navigation_ErrorStep, 'Ошибка при загрузке важных дат клиента: не найден id currentCustomerManaged'));
    }
};
/*
 * Thunk dispatched by "Customer" screen. Thunk chain used to show popover.
 */
export const performPopoverOccasionListShow = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    ...
    reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    if (reducerState.currentCustomerManaged.id != '') {
        dispatch(thunkOccasionList.performOpenOccasionListScreen(reducerState.currentCustomerManaged.occasionList, Object.assign({}, defaultValues.OccasionListConfig, { currentCustomer: reducerState.currentCustomerManaged, contextMode: Enums.OccasionListContextMode.CustomerManaged })));
    }
};
/*
 * Thunk dispatched by "Customer" screen. Thunk chain used to hide popover.
 */
export const performPopoverOccasionListHide = () => (dispatch) => {
    dispatch(actionsCustomer.performPopoverOccasionListHide());
};
/*
 * Thunk dispatched by "Customer" screen. Thunk chain used to show popover.
 */
export const performPopoverNoteListShow = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    ...
    state = getState();
    reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    dispatch(SplitPanelActions.pushContent(Enums.NavigationContentAgentCard.AgentNoteList, util.getNavigationAppCrmString(Enums.NavigationAppCRM.AgentScreen)));
    if (reducerState.currentCustomerManaged.id != '') {
        dispatch(thunkNoteList.navigateToNoteListScreen(reducerState.currentCustomerManaged.noteList, defaultValues.Agent, reducerState.currentCustomerManaged, Enums.NoteListContextMode.Customer, Enums.NoteListAccessLevel.Write));
    }
};
/*
 * Thunk dispatched by "Customer" screen. Thunk chain used to hide popover.
 */
export const performPopoverNoteListHide = () => (dispatch) => {
    dispatch(actionsCustomer.performPopoverNoteListHide());
};
/*
 * Thunk dispatched by "Customer" screen. Thunk chain used to show popover.
 */
export const performPopoverProblemListShow = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    let troubleCriteriaList = reducerState.currentCustomerManaged.troubleCriteriaList;
    if (troubleCriteriaList &&
 ...
 * Thunk dispatched by "Customer" screen. Thunk chain used to hide popover.
 */
export const performPopoverProblemListHide = () => (dispatch) => {
 ...
 */
export const performNavigateToOwnerScreen = (owner, customerMode) => (dispatch) => {
 ...
 * Thunk dispatched by "Customer" screen. Thunk chain used to show popover.
 */
export const performPopoverOwnerShow = (owner) => (dispatch) => {
    dispatch(actionsCustomer.performPopoverOwnerShow(owner));
};
/*
 * Thunk dispatched by "Customer" screen. Thunk chain used to hide popover.
 */
export const performPopoverOwnerHide = () => (dispatch) => {
 ...
 */
export const navigateToOwnerAgentScreen = (agent, agentContextMode = Enums.AgentContextMode.None) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    dispatch(thunkCustomer.performPopoverOwnerHide());
    ...
    state = getState();
    reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    dispatch(thunkAgent.performOpenAgentScreen(agent, reducerState.currentCustomerManaged, Enums.AgentMode.Watch, agentContextMode));
};
export const callGetCustomerActivityList = (customerActivityList) => (dispatch, getState) => {
    let state = getState();
    ...
    if (customerActivityList && customerActivityList.id) { //клиент не определен, поэтому запрос на получении списка задач не отправляется
        dispatch(performActivityListRefresh(contextMode, state.user.mrmkmcibCRM.reducerAppCRM.currentUser, defaultValues.Deal, Object.assign({}, customerActivityList), defaultValues.Agent));
    }
};
/*
 * Thunk dispatched by "Customer" screen. Thunk used to open customer screen.
 */
export const navigateAppDirectoryToCustomerScreen = (customerId) => (dispatch, getState) => {
    dispatch(thunkCustomer.navigateToCustomerScreen(Object.assign({}, defaultValues.Customer, { id: customerId }), Enums.CustomerMode.AppDirectoryBack, Enums.ShowCustomer.Show));
};
/*
 * Thunk dispatched by "Customer" screen. Thunk used to open customer screen.
 */
export const navigateToCustomerScreen = (customer, customerMode, showCustomer) => (dispatch, getState) => {
    const state = getState();
    ...
    dispatch(SplitPanelActions.resetSplitPanel(util.getNavigationContentCreditProduct(Enums.NavigationAppCRMCreditProduct.AppCRM_CreditProduct)));
    if (showCustomer && (customerMode !== Enums.CustomerMode.SameType && customerMode !== Enums.CustomerMode.AppScheduler)) {
        ...
        dispatch(HistoryMobileApp.pushContent({
            ...
            data: Object.assign({}, defaultValues.HistoryMobileAppData, { customer, qlikUrl, deal: Object.assign({}, defaultValues.Deal, currentDeal) })
        }));
    }
    const previousCustomer = state.user.mrmkmcibCRM.reducerCustomer.currentCustomerManaged;
    dispatch(actionsCustomer.navigateToCustomerScreen(Object.assign({}, defaultValues.CustomerManaged, customer), previousCustomer, customerMode, Enums.HistoryActions.Push, currentDealForecast, currentDeal));
    ...
    dispatch(thunkCustomer.customer_callGetCustomer(Object.assign({}, defaultValues.CustomerManaged, customer)));
};
export const navigateToCustomerScreenFromPush = () => (dispatch, getState) => {
    dispatch(SplitPanelActions.pushContent(Enums.NavigationContentAppCrm.AppCRM_Customer, util.getNavigationString(Enums.Navigation.AppCRM)));
};
export const getCustomerDataById = (id) => (dispatch, getState) => {
...
export const performRefresh = () => (dispatch, getState) => {
    ...
    const customer = reducerState.currentCustomerManaged.id != '' ?
        ...
        reducerState.currentCustomer.id != '' ?
            reducerState.currentCustomer : Object.assign({}, defaultValues.CustomerManaged, { id: reducerState.currentCustomerId });
...
export const performAgentListCacheReset = () => (dispatch, getState) => {
    let state = getState();
...
export const performCustomerCacheReset = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    ...
    Cache.sessionResetUrl(util.urlCustomer.callGetCustomer(state, util.getCustomerContextRequestString(customerContextMode, reducerState), [
 ...
 */
export const customer_callGetCustomer = (customer) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    if (reducerState.customerFetching) {
        return;
    ...
    if (!reducerState.customerFetching) {
        ...
        util.call(util.urlCustomer.callGetCustomer(state, customer.id, [{ tag: Enums.CachePolicy.Default }, { tag: util.getCustomerRequestRefreshString(Enums.CustomerRequestRefresh.CustomerRequestRefreshEnabled) }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerCustomer;
            if (reducerState.customer.id != operationId)
                return;
            ...
            if (!util.isCustomerManaged(reducerState.customer, state.user.mrmkmcibCRM.reducerAppCRM.currentUser)) {
                ...
                dispatch(thunkCustomer.customer_openCustomerManaged(Object.assign({}, defaultValues.CustomerManaged, response.data)));
            }
        ...
        }, Converters.RESPONSE_CUSTOMER_CUSTOMER_CALL_GET_CUSTOMER_TO_CUSTOMER_UNKNOWN, 'GET');
    }
};
/*
 * Internal thunk used in "Customer" container. Fetch current customer data by id.
 */
export const customer_callGetCustomerById = (customerId) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    if (!reducerState.customerFetching) {
        dispatch(actionsCustomer.customer_callGetCustomerRequest());
        util.call(util.urlCustomer.callGetCustomer(state, customerId, [{ tag: Enums.CachePolicy.Default }, { tag: util.getCustomerRequestRefreshString(Enums.CustomerRequestRefresh.CustomerRequestRefreshEnabled) }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerCustomer;
            dispatch(actionsCustomer.customer_callGetCustomerSuccess(response));
            // Dispatch thunk "customer_openCustomer" on fetch succeeded.
 ...
 */
export const customer_openCustomer = (customer) => (dispatch) => {
 ...
 */
export const customer_openCustomerManaged = (customerManaged) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    let reducerStateAppCRM = state.user.mrmkmcibCRM.reducerAppCRM;
    let currentUser = reducerStateAppCRM.currentUser;
 ...
 */
export const performPopoverLimitShow = () => (dispatch) => {
    dispatch(actionsCustomer.performPopoverLimitShow());
};
/*
 * Thunk dispatched by "Customer" screen. Thunk chain used to hide popover.
 */
export const performPopoverLimitHide = () => (dispatch) => {
 ...
 */
export const navigateToPopoverLimitItemScreen = (item) => (dispatch) => {
 ...
 */
export const navigatePopoverLimitBack = () => (dispatch) => {
 ...
 */
export const navigateToLimitScreen = () => (dispatch) => {
 ...
 */
export const callGetLimitNew = (refresh = false) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    ...
    if (!reducerState.limitFetching) {
        ...
        util.call(util.urlCustomer.callGetLimitNew(state, reducerState, [{ tag: Enums.CachePolicy.Default },
            { tag: util.getCustomerRequestRefreshString(Enums.CustomerRequestRefresh.CustomerRequestLimitRefreshEnabled) }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerCustomer;
            if (reducerState.currentCustomerId != operationId)
                return;
        ...
        }, Converters.RESPONSE_CUSTOMER_CALL_GET_LIMIT_NEW_TO_LIMIT, 'GET');
    }
};
/*
 * Thunk dispatched by "Customer" screen. Fetch current customer old limits.
 */
export const callGetLimitOld = (refresh = false) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    ...
    if (!reducerState.limitOldFetching) {
        ...
        util.call(util.urlCustomer.callGetLimitOld(state, reducerState, [{ tag: Enums.CachePolicy.Default },
            { tag: util.getCustomerRequestRefreshString(Enums.CustomerRequestRefresh.CustomerRequestLimitRefreshEnabled) }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerCustomer;
            if (reducerState.currentCustomerId != operationId)
        ...
        }, Converters.RESPONSE_CUSTOMER_CALL_GET_LIMIT_OLD_TO_LIMIT_OLD, 'GET');
    }
};
/*
 * Thunk dispatched by "Customer" screen. Thunk used to open customer new limit screen or old limit popover.
 */
export const performLimitShow = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
 ...
 */
export const performInputProductAgreementStatus = (agreementStatus) => (dispatch) => {
 ...
 */
export const performProductTypeListRefresh = () => (dispatch, getState) => {
 ...
 * Thunk dispatched by "Customer" screen. Fetch all product types data.
 */
export const performProductTypeListForceRefresh = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
 ...
 */
export const clearProductListCache = (productType, isActiveProductList) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    dispatch(actionsCustomer.performFlush());
    Cache.sessionResetTag({
...
const getFetchingCallGetRequestProductList = (reducerState, productType, isActiveProductList) => {
    switch (true) {
...
const getActionPerformCallGetRequestProductList = (productType, isActiveProductList) => {
    switch (true) {
        case (Enums.ProductType.CreditProduct === productType ||
            Enums.ProductType.BankGuaranteeProduct === productType):
            {
...
const getActionRequestCallGetRequestProductList = (reducerState, productType, isActiveProductList) => {
    switch (true) {
        case (Enums.ProductType.CreditProduct === productType ||
            Enums.ProductType.BankGuaranteeProduct === productType):
            {
...
const getConverterCallGetProductListResponse = (productType) => {
    switch (true) {
        case (Enums.ProductType.CreditProduct === productType ||
            Enums.ProductType.BankGuaranteeProduct === productType):
            {
...
export const getActionSuccessCallGetRequestProductList = (productType, isActiveProductList) => {
    switch (true) {
        case (Enums.ProductType.CreditProduct === productType ||
            Enums.ProductType.BankGuaranteeProduct === productType):
            {
...
const getActionFailureCallGetRequestProductList = (productType, isActiveProductList) => {
    switch (true) {
        case (Enums.ProductType.CreditProduct === productType ||
            Enums.ProductType.BankGuaranteeProduct === productType):
            {
...
const getPollingStatusCallGetRequestProductList = (reducerState, productType, isActiveProductList) => {
    switch (true) {
        case (Enums.ProductType.CreditProduct === productType ||
            Enums.ProductType.BankGuaranteeProduct === productType):
            {
...
export const performCallGetRequestProductList = (type, force, update) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
...
// Thunk dispatch in "Customer" screen to perform getting product list'
export const performCallGetForceRequestProductList = (type) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    let isActiveProductList = util.isActiveProductList(reducerState.productListAgreementStatus);
...
export const performCallGetCachedRequestProductList = (type) => (dispatch, getState) => {
    dispatch(performCallGetRequestProductList(type, false, false));
};
export const performCallGetUncachedRequestProductList = (type) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    let isActiveProductList = util.isActiveProductList(reducerState.productListAgreementStatus);
    dispatch(thunkCustomer.clearProductListCache(type, isActiveProductList));
...
const getProductListPollingStatusEnum = (reducerState, service, isActiveProductList) => {
    switch (service) {
...
export const updateProductListOperationUuid = (productType, isActiveProductList) => {
    switch (productType) {
...
const getProductListPollingOperationUuid = (reducerState, service, isActiveProductList) => {
    switch (service) {
        case Enums.ClientProductServiceList.AppCRM_Credit:
...
const callGetRequestProductList = (request) => (dispatch, getState) => {
    ...
    if (getPollingStatusCallGetRequestProductList(reducerState, type, active) != Enums.ProductPollingStatus.InProgress &&
    ...
    let operationId = reducerState.currentCustomerId;
    util.call(util.urlCustomer.callGetRequestProductList(state, active, getProductListPollingStatusEnum(reducerState, util.getProductListService(type), active), getProductListPollingOperationUuid(reducerState, util.getProductListService(type), active), util.getProductListPath(util.getProductListService(type)), reducerState.currentCustomerId, force, ttl, [
        { tag: cachePolicy },
        {
        ...
        dispatch(navigationExecutor(EnumsApp.NavigationStep.Navigation_ErrorStep, 'Ошибка при загрузке продуктов'));
    }, (data) => {
        state = getState();
        reducerState = state.user.mrmkmcibCRM.reducerCustomer;
        if (reducerState.currentCustomerId == operationId) {
            ...
            if (util.getProductListPollingStatus(data.status) == Enums.ProductPollingStatus.Success &&
                ...
                Cache.responsePersist(util.urlCustomer.callGetRequestProductList(state, active, Enums.ProductPollingStatus.Success, getProductListPollingOperationUuid(reducerState, util.getProductListService(type), active), util.getProductListPath(util.getProductListService(type)), reducerState.currentCustomerId, false, ttl, [
                    { tag: Enums.CachePolicy.Default },
                    {
                        tag: util.getTagCallGetRequestProductList(type, active),
                        contextId: reducerState.currentCustomerManaged.id
...
const getEksProductListFetching = (reducerState, productType, isActiveProductList) => {
    switch (true) {
        case (Enums.ProductType.CreditProduct === productType ||
            Enums.ProductType.BankGuaranteeProduct === productType):
            {
...
const getCallGetEksProductListAction = (productType, isActiveProductList) => {
    switch (true) {
        case (Enums.ProductType.CreditProduct === productType ||
            Enums.ProductType.BankGuaranteeProduct === productType):
            {
...
const getCallGetEksProductListRequest = (productType, isActiveProductList) => {
    switch (true) {
        case (Enums.ProductType.CreditProduct === productType ||
            Enums.ProductType.BankGuaranteeProduct === productType):
            {
...
const getCallGetEksProductListUrl = (productType) => {
    switch (true) {
        case Enums.ProductType.CreditProduct === productType:
...
const getCallGerEksProductListSuccess = (productType, isActiveProductList) => {
    switch (true) {
        case (Enums.ProductType.CreditProduct === productType ||
            Enums.ProductType.BankGuaranteeProduct === productType):
            {
...
const getCallGerEksProductListFailure = (productType, isActiveProductList) => {
    switch (true) {
        case (Enums.ProductType.CreditProduct === productType ||
            Enums.ProductType.BankGuaranteeProduct === productType):
            {
...
const getCallGetEksProductListPollingStatus = (reducerState, productType, isActiveProductList) => {
    switch (productType) {
        case Enums.ProductType.CreditProduct:
        case Enums.ProductType.BankGuaranteeProduct: return (isActiveProductList ? reducerState.creditActiveProductEksList.pollingStatus :
            reducerState.creditCloseProductEksList.pollingStatus);
        case Enums.ProductType.DepositProduct:
        case Enums.ProductType.ContractNsoProduct:
        case Enums.ProductType.ContractSdoProduct: return (isActiveProductList ? reducerState.depositActiveProductEksList.pollingStatus :
            reducerState.depositCloseProductEksList.pollingStatus);
        case Enums.ProductType.CorporateCardProduct: return (isActiveProductList ? reducerState.corporateCardActiveProductEksList.pollingStatus :
            reducerState.corporateCardCloseProductEksList.pollingStatus);
        case Enums.ProductType.EncashmentProduct:
        case Enums.ProductType.SelfEncashmentProduct: return (isActiveProductList ? reducerState.encashmentContractActiveProductEksList.pollingStatus :
            reducerState.encashmentContractCloseProductEksList.pollingStatus);
        case Enums.ProductType.CurrencyControlProduct: return (reducerState.legalInfoProductEksList.pollingStatus);
        case Enums.ProductType.AcquiringProduct: return (isActiveProductList ? reducerState.acquiringActiveProductEksList.pollingStatus :
            reducerState.acquiringCloseProductEksList.pollingStatus);
        case Enums.ProductType.DboProduct: return (isActiveProductList ? reducerState.dboActiveProductEksList.pollingStatus :
            reducerState.dboCloseProductEksList.pollingStatus);
        case Enums.ProductType.ContractConstructorProduct: return (isActiveProductList ? reducerState.udboActiveProductEksList.pollingStatus :
            reducerState.udboActiveProductEksList.pollingStatus);
        case Enums.ProductType.SalaryProduct: return (isActiveProductList ? reducerState.salaryActiveProductEksList.pollingStatus :
            reducerState.salaryCloseProductEksList.pollingStatus);
        case Enums.ProductType.CashManagementProduct:
        case Enums.ProductType.PaymentAccountProduct:
        case Enums.ProductType.PackageProduct:
        case Enums.ProductType.TariffPlanProduct:
        case Enums.ProductType.CustomsPaymentProduct: return (isActiveProductList ? reducerState.settlementAgreementActiveProductEksList.pollingStatus :
            reducerState.settlementAgreementCloseProductEksList.pollingStatus);
        default: return Enums.ProductPollingStatus.None;
    }
};
const getCallGetEksProductListConverterResponse = (productType) => {
    switch (true) {
        case (Enums.ProductType.CreditProduct === productType ||
            Enums.ProductType.BankGuaranteeProduct === productType):
            {
...
const getCallGetEksProductListConverterRequest = (productType) => {
    switch (true) {
        case (Enums.ProductType.CreditProduct === productType ||
            Enums.ProductType.BankGuaranteeProduct === productType):
            {
...
export const performUpdateProductList = (isActiveProductList, productType) => (dispatch, getState) => {
    let state = getState();
    ...
    switch (productType) {
        case Enums.ProductType.CreditProduct:
        case Enums.ProductType.BankGuaranteeProduct: {
            const updatedProductList = isActiveProductList ? Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.creditActiveProductList, { eksErrorList: reducerCustomer.creditActiveProductEksList.eksErrorList, data: [...reducerCustomer.creditActiveProductList.data,
                    ...reducerCustomer.creditActiveProductEksList.data] }) : Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.creditCloseProductList, { eksErrorList: reducerCustomer.creditCloseProductEksList.eksErrorList, data: [...reducerCustomer.creditCloseProductList.data,
        ...
        case Enums.ProductType.PackageProduct:
        case Enums.ProductType.TariffPlanProduct: {
            const updatedProductList = isActiveProductList ? Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.settlementAgreementActiveProductList, { eksErrorList: reducerCustomer.settlementAgreementActiveProductEksList.eksErrorList, data: [...reducerCustomer.settlementAgreementActiveProductList.data,
                    ...reducerCustomer.settlementAgreementActiveProductEksList.data] }) : Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.settlementAgreementCloseProductList, { eksErrorList: reducerCustomer.settlementAgreementCloseProductEksList.eksErrorList, data: [...reducerCustomer.settlementAgreementCloseProductList.data,
        ...
        case Enums.ProductType.ContractSdoProduct:
        case Enums.ProductType.ContractNsoProduct: {
            const updatedProductList = isActiveProductList ? Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.depositActiveProductList, { eksErrorList: reducerCustomer.depositActiveProductEksList.eksErrorList, data: [...reducerCustomer.depositActiveProductList.data,
                    ...reducerCustomer.depositActiveProductEksList.data] }) : Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.depositCloseProductList, { eksErrorList: reducerCustomer.depositCloseProductEksList.eksErrorList, data: [...reducerCustomer.depositCloseProductList.data,
        ...
        case Enums.ProductType.CorporateCardProduct: {
            const updatedProductList = isActiveProductList ? Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.corporateCardActiveProductList, { eksErrorList: reducerCustomer.corporateCardActiveProductEksList.eksErrorList, data: [...reducerCustomer.corporateCardActiveProductList.data,
                    ...reducerCustomer.corporateCardActiveProductEksList.data] }) : Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.corporateCardCloseProductList, { eksErrorList: reducerCustomer.corporateCardCloseProductEksList.eksErrorList, data: [...reducerCustomer.corporateCardCloseProductList.data,
        ...
        case Enums.ProductType.EncashmentProduct:
        case Enums.ProductType.SelfEncashmentProduct: {
            const updatedProductList = isActiveProductList ? Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.encashmentContractActiveProductList, { eksErrorList: reducerCustomer.encashmentContractActiveProductEksList.eksErrorList, data: [...reducerCustomer.encashmentContractActiveProductList.data,
                    ...reducerCustomer.encashmentContractActiveProductEksList.data] }) : Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.encashmentContractCloseProductList, { eksErrorList: reducerCustomer.encashmentContractCloseProductEksList.eksErrorList, data: [...reducerCustomer.encashmentContractCloseProductList.data,
        ...
        case Enums.ProductType.CurrencyControlProduct: {
            dispatch(getActionSuccessCallGetRequestProductList(productType, isActiveProductList)({
                ...
                data: Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.legalInfoProductList, { eksErrorList: reducerCustomer.legalInfoProductList.eksErrorList, data: [...reducerCustomer.legalInfoProductList.data,
                        ...reducerCustomer.legalInfoProductEksList.data] })
            }));
            break;
        }
        case Enums.ProductType.AcquiringProduct: {
            const updatedProductList = isActiveProductList ? Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.acquiringActiveProductList, { eksErrorList: reducerCustomer.acquiringActiveProductEksList.eksErrorList, data: [...reducerCustomer.acquiringActiveProductList.data,
                    ...reducerCustomer.acquiringActiveProductEksList.data] }) : Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.acquiringCloseProductList, { eksErrorList: reducerCustomer.acquiringCloseProductEksList.eksErrorList, data: [...reducerCustomer.acquiringCloseProductList.data,
        ...
        case Enums.ProductType.DboProduct: {
            const updatedProductList = isActiveProductList ? Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.dboActiveProductList, { eksErrorList: reducerCustomer.dboActiveProductEksList.eksErrorList, data: [...reducerCustomer.dboActiveProductList.data,
                    ...reducerCustomer.dboActiveProductEksList.data] }) : Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.dboCloseProductList, { eksErrorList: reducerCustomer.dboCloseProductEksList.eksErrorList, data: [...reducerCustomer.dboCloseProductList.data,
        ...
        case Enums.ProductType.ContractConstructorProduct: {
            const updatedProductList = isActiveProductList ? Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.udboActiveProductList, { eksErrorList: reducerCustomer.udboActiveProductEksList.eksErrorList, data: [...reducerCustomer.udboActiveProductList.data,
                    ...reducerCustomer.udboActiveProductEksList.data] }) : Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.udboCloseProductList, { eksErrorList: reducerCustomer.udboCloseProductEksList.eksErrorList, data: [...reducerCustomer.udboCloseProductList.data,
        ...
        case Enums.ProductType.SalaryProduct: {
            const updatedProductList = isActiveProductList ? Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.salaryActiveProductList, { eksErrorList: reducerCustomer.salaryActiveProductEksList.eksErrorList, data: [...reducerCustomer.salaryActiveProductList.data,
                    ...reducerCustomer.salaryActiveProductEksList.data] }) : Object.assign({}, state.user.mrmkmcibCRM.reducerCustomer.salaryCloseProductList, { eksErrorList: reducerCustomer.salaryCloseProductEksList.eksErrorList, data: [...reducerCustomer.salaryCloseProductList.data,
...
const getProductListOperationUuid = (reducerState, productType, isActiveProductList) => {
    switch (productType) {
        case Enums.ProductType.CreditProduct:
        case Enums.ProductType.BankGuaranteeProduct: return (isActiveProductList ? reducerState.creditActiveProductListOperationUuid
            : reducerState.creditCloseProductListOperationUuid);
        case Enums.ProductType.DepositProduct:
        case Enums.ProductType.ContractNsoProduct:
        case Enums.ProductType.ContractSdoProduct: return (isActiveProductList ? reducerState.depositActiveProductListOperationUuid
            : reducerState.depositCloseProductListOperationUuid);
        case Enums.ProductType.CorporateCardProduct: return (isActiveProductList ? reducerState.corporateCardActiveProductListOperationUuid
            : reducerState.corporateCardCloseProductListOperationUuid);
        case Enums.ProductType.EncashmentProduct:
        case Enums.ProductType.SelfEncashmentProduct: return (isActiveProductList ? reducerState.encashmentContractActiveProductListOperationUuid
            : reducerState.encashmentContractCloseProductListOperationUuid);
        case Enums.ProductType.CurrencyControlProduct: return (reducerState.legalInfoProductListOperationUuid);
        case Enums.ProductType.AcquiringProduct: return (isActiveProductList ? reducerState.acquiringActiveProductListOperationUuid
            : reducerState.acquiringCloseProductListOperationUuid);
        case Enums.ProductType.DboProduct: return (isActiveProductList ? reducerState.dboActiveProductListOperationUuid
            : reducerState.dboCloseProductListOperationUuid);
        case Enums.ProductType.ContractConstructorProduct: return (isActiveProductList ? reducerState.udboActiveProductListOperationUuid
            : reducerState.udboCloseProductListOperationUuid);
        case Enums.ProductType.SalaryProduct: return (isActiveProductList ? reducerState.salaryActiveProductListOperationUuid
            : reducerState.salaryCloseProductListOperationUuid);
        case Enums.ProductType.CashManagementProduct:
        case Enums.ProductType.PaymentAccountProduct:
        case Enums.ProductType.PackageProduct:
        case Enums.ProductType.TariffPlanProduct:
        case Enums.ProductType.CustomsPaymentProduct: return (isActiveProductList ? reducerState.settlementAgreementActiveProductListOperationUuid
            : reducerState.settlementAgreementCloseProductListOperationUuid);
        default: return "";
    }
};
export const performCallEksRequestProductList = (productType, productListStatus) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    ...
    reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    util.call(getCallGetEksProductListUrl(productType)(state, reducerState, [{ tag: Enums.CachePolicy.Disabled }], productListStatus), (response) => {
        state = getState();
        reducerState = state.user.mrmkmcibCRM.reducerCustomer;
        if (reducerState.currentCustomerId != operationId)
            return;
        ...
        if (getCallGetEksProductListPollingStatus(reducerState, productType, isActiveProductList) ==
            Enums.ProductPollingStatus.InProgress) {
            setTimeout((productType, productListStatus) => {
                state = getState();
                reducerState = state.user.mrmkmcibCRM.reducerCustomer;
                if (reducerState.currentCustomerId != operationId)
                    return;
    ...
    }, getCallGetEksProductListConverterResponse(productType), getCallGetEksProductListPollingStatus(reducerState, productType, isActiveProductList) == Enums.ProductPollingStatus.InProgress ?
        'GET' :
        'POST', { 'Content-type': 'application/json; charset=UTF-8',
        'operUid': getProductListOperationUuid(reducerState, productType, isActiveProductList) }, getCallGetEksProductListConverterRequest(productType), { data: util.getEksErrorProductList(reducerState, productType, isActiveProductList) });
};
/*
 * Thunk dispatched by "Customer" screen. Thunk chain used to include company to Customer.
 */
export const navigateToCustomerActivityIncludeScreen = () => (dispatch) => {
    dispatch(SplitPanelActions.pushContent(Enums.NavigationCustomerAccessory.AppCRM_CustomerEditor_Include, util.getNavigationContentString(Enums.NavigationContentAppCrm.AppCRM_Customer)));
 ...
 */
export const navigateToCustomerActivityExcludeScreen = () => (dispatch) => {
    dispatch(SplitPanelActions.pushContent(Enums.NavigationCustomerAccessory.AppCRM_CustomerEditor_Exclude, util.getNavigationContentString(Enums.NavigationContentAppCrm.AppCRM_Customer)));
    dispatch(actionsCustomer.navigateToCustomerActivityExcludeScreen());
};
export const navigateToCustomerActivityAccessScreen = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    let classifiers = state.user.mrmkmcibCRM.reducerAppCRM.classifierDictionary;
    const priority = _.find(classifiers.ACTIVITY_PRIORITY.data, item => item.code == "1-ASAP") || ModelsAppDefault.Classifier;
    const activityType = _.find(shedulerUtil.getActivityAvailableTypeList().data, item => item.code == "Task") || ModelsAppDefault.Classifier;
    dispatch(performInputActivity(EnumsScheduler.SchedulerMode.Customer, Object.assign({}, ModelsSheduler.defaultValues.Activity, { customer: reducerState.currentCustomer, description: 'Задача на предоставление доступа', priority: priority, type: activityType, memberList: { data: reducerState.currentCustomer.holder ?
                [Object.assign({}, reducerState.currentCustomer.holder, { isGeneral: true })] : [] } }), EnumsScheduler.ActivityMode.Create, EnumsScheduler.ActivityContextMode.CustomerAccess));
 ...
 */
export const closeCustomerActivityPanel = () => (dispatch) => {
 ...
 * Thunk dispatched by "Customer" screen.
 */
export const callGetForecastDealList = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    ...
    util.call(util.urlProductCredit.callGetForecastDealList(state, reducerState, [{ tag: Enums.CachePolicy.Disabled }], false, 600), (response) => {
        state = getState();
    ...
    }, Converters.RESPONSE_CREDIT_CALL_GET_FORECAST_DEAL_LIST_TO_FORECAST_DEAL_LIST, 'GET');
};
export const navigateToCurrentCustomerAgentScreen = (agentId) => (dispatch, getState) => {
    let state = getState();
...
export const performCustomerActivityListRefresh = (customerId) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    ...
    let customerForActivityList = customerSearchList && Array.isArray(customerSearchList.data) &&
        customerSearchList.data.length > 0
        ? customerSearchList
            .data.find((customerManaged) => customerManaged.id == customerId)
        : undefined;
    if (customerForActivityList == undefined) {
        customerForActivityList = customerManagedList && Array.isArray(customerManagedList.data)
            ? customerManagedList.data.find((customerManaged) => customerManaged.id == customerId) || Object.assign({}, defaultValues.Customer, { id: customerId })
            : undefined;
    }
    ...
    dispatch(thunkCustomer.callGetCustomerActivityList(Object.assign({}, defaultValues.CustomerManaged, customerForActivityList)));
};
export const fillOccasionListContent = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    if (reducerState.currentCustomerManaged.id != '') {
        dispatch(thunkOccasionList.performOpenOccasionListScreen(reducerState.currentCustomerManaged.occasionList, Object.assign({}, defaultValues.OccasionListConfig, { contextMode: Enums.OccasionListContextMode.Notice, currentCustomer: reducerState.currentCustomerManaged })));
        dispatch(navigationExecutor(EnumsApp.NavigationStep.Occasion_PerformRefreshOccasionSuccess));
    }
    else {
        dispatch(navigationExecutor(EnumsApp.NavigationStep.Navigation_ErrorStep, 'Ошибка при загрузке важных дат клиента: не найден id currentCustomerManaged'));
...
export const performRefreshCustomerActivitylist = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
...
export const performGetProductFromPush = (getProductFromPushData) => (dispatch, getState) => {
    let state = getState();
    ...
    let statusRequestMaxPeriod = Number(state.user.mrmkmcibApp.reducerAuthorization.currentConfiguration[supParamNames().StatusRequestMaxPeriod]);
    util.call(util.productPostUrl.callGetProductList(state, util.getProductListService(productType), customerId, [{ tag: Enums.CachePolicy.Disabled }]), (response) => {
    ...
    }, 'POST', { 'Content-type': 'application/json; charset=UTF-8' }, Converters.REQUEST_PRODUCTS_CALL_POST_REQUEST, {
        ids: [{ id: eksId, isActive, reason: 'push' }]
    });
};
export const checkProductStatus = (getProductFromPushData, oid) => (dispatch, getState) => {
    let state = getState();
    const { productId, productType, customerId, eksId, isActive, isLast } = getProductFromPushData;
    let statusRequestMaxPeriod = Number(state.user.mrmkmcibApp.reducerAuthorization.currentConfiguration[supParamNames().StatusRequestMaxPeriod]);
    util.call(util.productPostUrl.getProductStatus(state, util.getProductListService(productType), oid, [{ tag: Enums.CachePolicy.Disabled }]), (response) => {
        switch (response.data.pollingStatus) {
            case Enums.ProductPollingStatus.InProgress: {
...
export const findProductFromList = (getProductFromPushData) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
        ...
        case Enums.ProductType.PaymentAccountProduct: {
            ...
            if (productList.length > 0) {
                product = productList.find(product => {
                    return (product.paymentAccountProduct != null &&
...
export const checkIsLastCall = (getProductFromPushData) => (dispatch, getState) => {
    const { isLast } = getProductFromPushData;
    if (!isLast) {
        dispatch(thunkCustomer.performGetProductFromPush(Object.assign({}, getProductFromPushData, { isActive: false, isLast: true })));
    }
    else {
        dispatch(navigationExecutor(EnumsApp.NavigationStep.Navigation_ErrorStep, 'Ошибка при загрузке продукта'));
    }
};
export const performPrognosticProductListModalAlertHide = () => (dispatch) => {
    dispatch(actionsCustomer.performPrognosticProductListModalAlertHide());
};
export const performPrognosticProductListModalAlertShow = () => (dispatch) => {
    dispatch(actionsCustomer.performPrognosticProductListModalAlertShow());
};
/*
 * Thunk dispatched by "Customer" screen.
 */
export const callGetForecastPrognosticDealList = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomer;
    const operationId = state.user.mrmkmcibCRM.reducerCustomer.currentCustomerManaged.id;
    if (reducerState.forecastDealListFetching) {
    ...
    util.call(util.urlProductCredit.callGetPrognosticForecastDealList(state, reducerState, [
...
export const setActivityAccessError = (isActivityAccessError) => (dispatch) => {
    dispatch(actionsCustomer.performSetActivityAccessError(isActivityAccessError));
};
//# sourceMappingURL=ThunkCustomer.js.map