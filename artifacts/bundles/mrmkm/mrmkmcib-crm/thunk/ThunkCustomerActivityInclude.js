...
export const navigateToMemberListScreenPage = () => (dispatch, getState) => {
    ...
    dispatch(SplitPanelActions.pushContent(Enums.NavigationIntoExcludeIncludeView.IncludeExclude_MemberList, util.getNavigationCustomerEditInIncludeExcludeOrganizationString(Enums.NavigationCustomerEditInIncludeExcludeOrganization.CustomerEditor_Include_View)));
    dispatch(thunkMemberList.performMemberListRefresh(Object.assign({}, defaultValues.Activity, { memberList: reducerMemberList }), defaultValues.Deal, state.user.mrmkmcibCRM.reducerCustomer.currentCustomerManaged, defaultValues.Gsz, defaultValues.Agent, true, Enums.MemberListMode.CustomerActivityRemoveOrganization));
...
export const search_callGetCustomerSearchList = () => (dispatch, getState) => {
    ...
    if (!reducerState.customerSearchListFetching) {
        ...
        util.call(util.urlCustomerActivityInclude.search_callGetCustomerSearchList(state, reducerState, [{ tag: Enums.CachePolicy.Disabled }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerCustomerActivityInclude;
...
export const callGetCustomer = () => (dispatch, getState) => {
    ...
    if (!reducerState.inputCustomerFetching) {
        ...
        util.call(util.urlCustomerActivityInclude.callGetCustomer(state, reducerState, [{ tag: Enums.CachePolicy.Default }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerCustomerActivityInclude;
            if (state.user.mrmkmcibCRM.reducerCustomer.currentCustomerManaged.id != operationId)
                return;
            ...
            dispatch(actionsCustomerActivityInclude.performChangeDisplayErrorModalWindow(true));
        }, Converters.RESPONSE_CUSTOMER_ACTIVITY_INCLUDE_CALL_GET_CUSTOMER_TO_CUSTOMER, 'GET');
    }
};
/*
 ...
 */
export const callPostCustomerActivityIncludeCreate = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerCustomerActivityInclude;
    ...
    if (!reducerState.activityIncludeCreateFetching) {
        ...
        util.call(util.urlCustomerActivityInclude.callPostCustomerActivityIncludeCreate(state, reducerState, [{ tag: Enums.CachePolicy.Disabled }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerCustomerActivityInclude;
            if (state.user.mrmkmcibCRM.reducerCustomer.currentCustomerManaged.id != operationId)
                return;
        ...
        }, Converters.RESPONSE_CUSTOMER_ACTIVITY_INCLUDE_CALL_POST_CUSTOMER_ACTIVITY_INCLUDE_CREATE_TO_BOOLEAN, 'POST', { 'Content-type': 'application/json; charset=UTF-8', 'operuid': reducerState.operationUuid }, Converters.REQUEST_CUSTOMER_ACTIVITY_INCLUDE_CALL_POST_CUSTOMER_ACTIVITY_INCLUDE_CREATE_FROM_CUSTOMER_ACTIVITY_INCLUDE_CREATE_REQUEST, {
...
//# sourceMappingURL=ThunkCustomerActivityInclude.js.map