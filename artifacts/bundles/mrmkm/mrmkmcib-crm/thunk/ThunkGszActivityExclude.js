...
export const search_callGetCustomerSearchList = () => (dispatch, getState) => {
    ...
    if (!reducerState.customerSearchListFetching) {
        ...
        util.call(util.urlGszActivityExclude.search_callGetCustomerSearchList(state, reducerState, [{ tag: Enums.CachePolicy.Default }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerGszActivityExclude;
        ...
        }, Converters.RESPONSE_GSZ_ACTIVITY_EXCLUDE_SEARCH_CALL_GET_CUSTOMER_SEARCH_LIST_TO_CUSTOMER_LIST, 'GET');
    }
};
/*
 ...
 */
export const callPostGszActivityExcludeCreate = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerGszActivityExclude;
    ...
    if (!reducerState.activityExcludeCreateFetching) {
        ...
        util.call(util.urlGszActivityExclude.callPostGszActivityExcludeCreate(state, reducerState, [{ tag: Enums.CachePolicy.Disabled }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerGszActivityExclude;
            if (state.user.mrmkmcibCRM.reducerGSZ.currentGszId != operationId)
                return;
        ...
        }, Converters.RESPONSE_GSZ_ACTIVITY_EXCLUDE_CALL_POST_GSZ_ACTIVITY_EXCLUDE_CREATE_TO_BOOLEAN, 'POST', { 'Content-type': 'application/json; charset=UTF-8', 'operUid': reducerState.operationUuid }, Converters.REQUEST_GSZ_ACTIVITY_EXCLUDE_CALL_POST_GSZ_ACTIVITY_EXCLUDE_CREATE_FROM_GSZ_ACTIVITY_EXCLUDE_CREATE_REQUEST, {
...
export const navigateToMemberListScreen = () => (dispatch, getState) => {
    ...
    dispatch(SplitPanelActions.pushContent(Enums.NavigationIntoExcludeIncludeView.IncludeExclude_MemberList, util.getNavigationGszEditInIncludeExcludeOrganizationString(Enums.NavigationGszEditInIncludeExcludeOrganization.Gsz_CustomerEditor_Exclude_View)));
    dispatch(thunkMemberList.performContainerReset());
    dispatch(thunkMemberList.performMemberListRefresh(Object.assign({}, defaultValues.Activity, { memberList: reducerMemberList }), defaultValues.Deal, defaultValues.CustomerManaged, gszReducerState.currentGsz, defaultValues.Agent, true, Enums.MemberListMode.CustomerActivityRemoveGSZ));
...
//# sourceMappingURL=ThunkGszActivityExclude.js.map