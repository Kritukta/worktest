 ...
 */
import { showAlert, supParamNames } from "mrmkmcib-app";
import { performReturnToActivityScreen, swapContextMemberList, EnumsScheduler } from "mrmkmcib-scheduler";
 ...
 */
export const performMemberListRefresh = (currentActivity, currentDeal, currentCustomerManaged, currentGsz, currentAgent, isExpandedMode, currentMode, inputEmployeeSource) => (dispatch, getState) => {
    let state = getState();
    ...
    if (reducerState.currentMode == Enums.MemberListMode.Deal) {
        ...
        const isEditorEnabled = reducerState.memberList.data.find((item) => {
...
export const setEditingPermissionMemberList = () => (dispatch, getState) => {
    let state = getState();
...
export const navigateBackFromMembersSearchPage = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
...
export const navigateSearchMemberListScreenBack = () => (dispatch, getState) => {
 ...
 */
export const swapContext = (memberList) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    dispatch(actionsMemberList.swapContext(memberList));
};
export const performSetEditorEnabled = (isEnabled) => (dispatch, getState) => {
 ...
 */
export const navigateToMemberSearchScreen = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    dispatch(SplitPanelActions.pushContent(Enums.MemberList.AppCRM_MemberList_Search, util.getNavigationPopoverMembersString(Enums.MemberList.AppCRM_MemberList_Search)));
    dispatch(actionsMemberList.navigateToMemberSearchScreen());
};
/*
 * Thunk dispatched by "MemberList" screen. Thunk used to hide add menu.
 */
export const navigateToMemberSearchLocalScreen = (employeeSource, isSelectEmployeeLocalSearch) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    dispatch(actionsMemberList.performSetSelectEmployeeLocalSearch(employeeSource, isSelectEmployeeLocalSearch));
    dispatch(SplitPanelActions.pushContent(Enums.MemberList.AppCRM_MemberList_Search_Local, util.getNavigationPopoverMembersString(Enums.MemberList.AppCRM_MemberList_Search)));
    dispatch(actionsMemberList.navigateToMemberSearchLocalScreen());
};
export const navigateMemberListScreenBack = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    switch (reducerState.currentMode) {
        case Enums.MemberListMode.Activity: {
 ...
 */
export const performInputMemberSearch = (value) => (dispatch) => {
...
export const performResetMemberSearchList = () => (dispatch, getState) => {
 ...
 */
export const performInputMemberSearchLocal = (value) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
...
const getEmployeeMode = (currentMode) => {
    switch (currentMode) {
        case Enums.MemberListMode.CustomerManaged: {
...
export const filterMemberSearchListLocal = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    let lastInput = reducerState.inputMemberSearchLocal;
            ...
            switch (reducerState.inputEmployeeSource) {
                case Enums.MemberListEmployeeSource.CustomerManaged: {
                    if (lastInput && lastInput.trim().length > 0) {
                        const result = reducerCustomer.currentCustomerManaged.memberList.data.filter((item) => {
                            return util.filterMemberList(item.firstName, item.lastName, item.middleName, item.positionName, item.role.value, item.isGeneral, lastInput);
                        });
                ...
                default: {
                    if (lastInput && lastInput.trim().length > 0) {
                        const result = reducerState.memberList.data.filter((item) => {
                            return util.filterMemberList(item.firstName, item.lastName, item.middleName, item.positionName, item.role.value, item.isGeneral, lastInput);
                        });
                        dispatch(actionsMemberList.filterInputMemberSearchLocal({ data: result }));
                    }
 ...
 */
export const callGetMemberSearchList = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    ...
    const isEmployeeRequest = reducerState.currentMode == Enums.MemberListMode.Gsz ||
    ...
    if (!reducerState.memberSearchListFetching && personSearchType) {
        ...
        util.call(getUrl(state, reducerState, [{ tag: Enums.CachePolicy.Disabled }]), (response) => {
            let existingPositionsList = { data: [] };
            switch (reducerState.currentMode) {
                case Enums.MemberListMode.Deal:
                case Enums.MemberListMode.DealEditor:
                case Enums.MemberListMode.DealStageMainClientManager:
            ...
            : Converters.RESPONSE_MEMBER_LIST_CALL_GET_MEMBER_SEARCH_LIST_TO_MEMBER_LIST, 'POST', { 'Content-type': 'application/json; charset=UTF-8' }, Converters.REQUEST_MEMBER_LIST_CALL_GET_MEMBER_SEARCH_LIST_FROM_AGENT_SEARCH_LIST_REQUEST, {
 ...
 */
export const navigateToMemberRolePickerScreen = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    dispatch(SplitPanelActions.pushContent(Enums.MemberList.AppCRM_MemberList_SelectRole, util.getNavigationPopoverMembersString(Enums.MemberList.AppCRM_MemberList_List)));
 ...
 */
export const performMemberSearchListSelect = (member) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    ...
    let selectedMemeber = Object.assign({}, member);
    ...
    switch (reducerState.currentMode) {
        ...
        case Enums.MemberListMode.DealStageMainClientManager: {
            ...
            selectedMemeber.role = classifierDictionary.SALES_TEAM_ROLE.data.find((element) => {
...
export const performSelectGeneralMember = (index) => (dispatch, getState) => {
    dispatch(actionsMemberList.performGeneralMemberSelect(index));
    dispatch(SplitPanelActions.popContent(util.getNavigationPopoverMembersString(Enums.MemberList.AppCRM_MemberList_SelectRole)));
};
/*
 * Thunk dispatched by "MemberList" screen. Thunk used to navigate to general member picker screen
 */
export const navigateToSelectMemberIsGeneralScreen = () => (dispatch, getState) => {
    dispatch(actionsMemberList.navigateToSelectMemberIsGeneralScreen());
    dispatch(SplitPanelActions.pushContent(Enums.MemberList.AppCRM_MemberList_Select_IsGeneral, util.getNavigationPopoverMembersString(Enums.MemberList.AppCRM_MemberList_SelectRole)));
};
 ...
 * Thunk dispatched by "MemberList" screen. Thunk used to select member from search list.
 */
export const performMemberRoleSelect = (role) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
  ...
  */
        state = getState();
        reducerState = state.user.mrmkmcibCRM.reducerMemberList;
        dispatch(SplitPanelActions.pushContent(Enums.MemberList.AppCRM_MemberList_List, util.getNavigationPopoverMembersString(Enums.MemberList.AppCRM_MemberList_List)));
    };
    state = getState();
    reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    switch (reducerState.currentMode) {
        ...
        case Enums.MemberListMode.Deal: {
            ...
            if (reducerState.currentDeal.requestTypeClassifier &&
                ...
                reducerState.memberList.data.some((element) => {
                ...
                const isMemberWithRolePresent = reducerState.memberList.data.find((item) => item.role && item.role.value === role.value);
 ...
 * @returns {boolean}
 */
const activityJustStoreInStateNecessary = (state) => (state.user.mrmkmcibScheduler.reducerActivity.activityMode === EnumsScheduler.ActivityMode.Create);
/*
...
export const callPutActivityMemberListUpdate = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    ...
    if (!reducerState.activityMemberListFetching) {
        ...
        util.call(util.urlMemberList.callPutActivityMemberListUpdate(state, reducerState, [{ tag: Enums.CachePolicy.Disabled }]), (response) => {
        ...
        }, Converters.RESPONSE_MEMBER_LIST_CALL_PUT_ACTIVITY_MEMBER_LIST_TO_BOOLEAN, 'PUT', {
...
export const performSaveActivityMemberList = () => (dispatch, getState) => {
    dispatch(actionsMemberList.performSaveActivityMemberList());
};
/*
 * Thunk dispatched by "MemberList" screen. Fetch put request.
 */
export const callPutDealMemberListUpdate = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    ...
    if (!reducerState.memberListUpdateFetching) {
        ...
        util.call(util.urlMemberList.callPutDealMemberListUpdate(state, reducerState, [{ tag: Enums.CachePolicy.Disabled }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerMemberList;
            if (reducerState.currentDeal.id != operationId)
                return;
            ...
            dispatch(thunkMemberList.performSaveFailure(error));
        }, Converters.RESPONSE_MEMBER_LIST_CALL_PUT_DEAL_MEMBERL_LIST_UPDATE_TO_BOOLEAN, 'PUT', { 'Content-type': 'application/json; charset=UTF-8' }, Converters.REQUEST_MEMBER_LIST_CALL_PUT_DEAL_MEMBERL_LIST_UPDATE_FROM_DEAL_UPDATE_REQUEST, {
...
export const callGetDealMemberListRefresh = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    ...
    if (!reducerState.currentDealMemberListFetching) {
        ...
        util.call(util.urlMemberList.callGetDealMemberListRefresh(state, reducerState, [
 ...
 * Thunk dispatched by "MemberList" screen. Fetch put request.
 */
export const callPutAgentMemberListUpdate = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    if (reducerState.memberListUpdateFetching) {
        return;
    ...
    if (!reducerState.memberListUpdateFetching) {
        dispatch(actionsMemberList.callPutMemberListUpdateRequest());
        util.call(util.urlMemberList.callPutAgentMemberListUpdate(state, reducerState, [{ tag: Enums.CachePolicy.Disabled }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerMemberList;
            if (reducerState.currentDeal.id != operationId)
                return;
            ...
            // Dispatch thunk "performSaveFailure" on fetch failure.
            dispatch(thunkMemberList.performSaveFailure(error));
        }, Converters.RESPONSE_MEMBER_LIST_CALL_PUT_AGENT_MEMBERL_LIST_UPDATE_TO_BOOLEAN, 'PUT', { 'Content-type': 'application/json; charset=UTF-8' }, Converters.REQUEST_MEMBER_LIST_CALL_PUT_AGENT_MEMBERL_LIST_UPDATE_FROM_AGENT_UPDATE_REQUEST, {
            operationUuid: reducerState.operationUuid,
 ...
 * Thunk dispatched by "MemberList" screen. Fetch put request.
 */
export const callPutMemberListUpdate = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    if (reducerState.memberListUpdateFetching) {
        return;
    ...
    if (!reducerState.memberListUpdateFetching) {
        dispatch(actionsMemberList.callPutMemberListUpdateRequest());
        util.call(util.urlMemberList.callPutMemberListUpdate(state, reducerState, [{ tag: Enums.CachePolicy.Disabled }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerMemberList;
            ...
            // Dispatch thunk "performSaveFailure" on fetch failure.
            dispatch(thunkMemberList.performSaveFailure(error));
        }, Converters.RESPONSE_MEMBER_LIST_CALL_PUT_CUSTOMER_UPDATE_TO_BOOLEAN, 'PUT', { 'Content-type': 'application/json; charset=UTF-8' }, Converters.REQUEST_MEMBER_LIST_CALL_PUT_CUSTOMER_UPDATE_FROM_CUSTOMER_UPDATE_REQUEST, {
            operationUuid: reducerState.operationUuid,
...
export const callGetCustomerModifierId = () => (dispatch, getState) => {
    ...
    if (!reducerState.customerModifierIdFetching) {
        ...
        util.call(util.urlMemberList.callGetCustomerModifierId(state, reducerState, [{ tag: Enums.CachePolicy.Disabled }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerMemberList;
            if (reducerState.currentCustomerManaged.id != operationId)
                return;
 ...
 */
export const performMemberDelete = (index) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
 ...
 */
export const performMemberActionMenuSwitch = (id, isLeftActionMenuOpen, isRightActionMenuOpen) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
...
export const performResetAndReturn = () => (dispatch, getState) => {
 ...
 * Thunk dispatched by "MemberList" screen.
 */
export const navigateGszActivityIncludeExcludeBack = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    if (reducerState.currentMode == Enums.MemberListMode.CustomerActivityAddGSZ) {
...
export const setInfoButtonFlag = (flag) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
 ...
 */
export const performNavigateToMemberScreen = (memberId) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    ...
    switch (reducerState.currentMode) {
        case Enums.MemberListMode.DealEditor: {
            dispatch(SplitPanelActions.pushContent(Enums.NavigationContentDealEditor.DealEditorEmploee, util.getNavigationContentString(Enums.NavigationContentAppCrm.AppCRM_DealEditor)));
            break;
        }
        case Enums.MemberListMode.Deal: {
        ...
        case Enums.MemberListMode.Agent: {
            dispatch(SplitPanelActions.pushContent(Enums.NavigationContentAgentCard.AgentEmployee, util.getNavigationAppCrmString(Enums.NavigationAppCRM.AgentScreen)));
            break;
        }
        case Enums.MemberListMode.DealStageMainCreditOfficer:
        case Enums.MemberListMode.DealStageMemberMonitoring:
        case Enums.MemberListMode.DealStageMainClientManager: {
            dispatch(SplitPanelActions.pushContent(Enums.MemberList.AppCRM_MemberList_Member, util.getNavigationContentString(Enums.NavigationContentAppCrm.AppCRM_MemberList)));
            break;
        }
        default: {
            dispatch(SplitPanelActions.pushContent(Enums.MemberList.AppCRM_MemberList_Member, util.getNavigationPopoverMembersString(Enums.MemberList.AppCRM_MemberList_Member)));
        }
    }
...
export const performSetMemberSearchList = (data) => (dispatch, getState) => {
    dispatch(actionsMemberList.performSetMemberSearchList(data));
};
export const performSetMemberSearchListEmpty = () => (dispatch, getState) => {
    dispatch(actionsMemberList.performSetMemberSearchListEmpty());
};
export const setLocalSearchFlag = (isLocalSearch) => (dispatch, getState) => {
    dispatch(actionsMemberList.setLocalSearchFlag(isLocalSearch));
};
export const performInputEmployeeSourceActivity = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
...
export const callGetCustomerForActivity = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    ...
    if (!reducerState.customerForActivityFetching) {
        ...
        util.call(util.urlMemberList.callGetCustomerForActivity(state, customerId, [{ tag: Enums.CachePolicy.Disabled }]), (response) => {
...
export const performInputEmployeeSource = (inputEmployeeSource) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
    dispatch(thunkMemberList.performPopoverAddHide());
 ...
 */
export const navigateToActivityBack = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
 ...
 */
export const swapContextMemberListToActivity = (memberList) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerMemberList;
...
export const performCancelSaveMemberListError = () => (dispatch, getState) => {
    dispatch(actionsMemberList.performCancelSaveMemberListError());
};
export const performSetDealCardOwner = (isDealCardOwner) => (dispatch, getState) => {
    dispatch(actionsMemberList.performSetDealCardOwner(isDealCardOwner));
};
//# sourceMappingURL=ThunkMemberList.js.map