...
import { EnumsApp, performNavigateToEntity, supParamNames, performNavigateBack, } from 'mrmkmcib-app';
 ...
 */
export const shareDataRefresh = (message) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibDashboard.reducerCustomerDashboard;
 ...
 */
export const callQlikAvailabilityCheck = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibDashboard.reducerCustomerDashboard;
    ...
    if (!reducerState.qlikAvailabilityCheckFetching) {
        ...
        util.ping(util.urlCustomerDashboard.callQlikAvailabilityCheck(state, reducerState, [{ tag: Enums.CachePolicy.Disabled }]), (response) => {
 ...
 */
export const swapContext = (customerManaged, user) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibDashboard.reducerCustomerDashboard;
...
export const performQlikEvent = (message) => (dispatch, getState) => {
    ...
    const navigateBackData = Object.assign({}, defaultValues.NavigateToEntity, { navigationType: EnumsApp.NavigationType.CustomerDashboard, customerId: state.user.mrmkmcibCRM.reducerCustomer.currentCustomerId, qlikUrl: message.payload.currentUrl });
    switch (message.type) {
...
export const performUpdateSupParameters = () => (dispatch, getState) => {
    ...
    Settings
        ...
        .catch((error) => {
        dispatch(actionsCustomerDashboard.performUpdateSupParametersFailure(Object.assign({}, defaultValues.Error)));
        // Dispatch thunk "callQlikAvailabilityCheck" synchronously.
        dispatch(thunkCustomerDashboard.callQlikAvailabilityCheck());
    });
};
 ...
 */
export const setCurrentCustomerDashboardQlikUrl = (url) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibDashboard.reducerCustomerDashboard;
...
export const performFindPeople = () => (dispatch, getState) => {
    ...
    OWAService.findPeople(reducerState.inputSharePopoverSearch)
        ...
        .catch((error) => {
        dispatch(thunkCustomerDashboard.performFindPeopleFailure(Object.assign({}, defaultValues.Error)));
    });
    // }
};
 ...
 */
export const setTrimmedTop = (value) => (dispatch, getState) => {
...
export const setQlikCookie = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibDashboard.reducerAppDashboard;
    Cookie.get((cookies) => {
...
//# sourceMappingURL=ThunkCustomerDashboard.js.map