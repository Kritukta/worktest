...
/**
 * `key` - уникальный ключ страницы относительно страниц в одном `ContentPanel` или `AccessoryPanel`.
 * Используется для навигационных переходов в методах `popAccessoryToPage` и `popContentToPage`.
 *
 * `data` - произвольный объект с полезной информацией.
 */
