...
export const responsePersist = (url: Models.Url, data: any, header?: any): void => {
    if (url.tagList) {
    ...
    if (__DEV__) console.log('Response persisted to cache.', url, header, data, )
...
export const sessionResetUrl = (url: Models.Url): void => {
    currentCacheSession.store = currentCacheSession.store.filter((cacheRequest) => {
