...
import {Enums as EnumsAppAll, HistoryMobileApp} from "mrmkmcib-app"
...
export const navigateToCustomerPickerScreen = () => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    ...
    dispatch(SplitPanelActions.pushContent(Enums.NavigationIntoExcludeIncludeView.IncludeExclude_ClientsSearch,
        util.getNavigationCustomerEditInIncludeExcludeOrganizationString(Enums.NavigationCustomerEditInIncludeExcludeOrganization.CustomerEditor_Exclude_View)))
...
export const getNavigationContentSchedulerActivity = (page:Enums.NavigationActivityDetails) =>
{
    switch (page) {
...
export const navigateToMemberListScreenPage = () => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    ...
    dispatch(SplitPanelActions.pushContent(Enums.NavigationIntoExcludeIncludeView.IncludeExclude_MemberList,
        ...
        dispatch(thunkMemberList.performMemberListRefresh(            
            ...
            defaultValues.Agent,
            true,
            Enums.MemberListMode.CustomerActivityAddOrganization))
...
export const search_callGetCustomerSearchList = () => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    ...
    if (!reducerState.customerSearchListFetching) {
        ...
        util.call<Models.CustomerList, void>(
            util.urlCustomerActivityExclude.search_callGetCustomerSearchList(state, reducerState, [{tag: Enums.CachePolicy.Disabled}]),
 ...
 */
export const filterCustomerList = (inputSearch: string, inputSearchManagedOnly: boolean, currentCustomerManaged: Models.CustomerManaged,currentCustomer: Models.Customer) => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    let state = getState()
    let reducerState = state.user.mrmkmcibCRM.reducerCustomerActivityExclude
...
export const callGetCustomer = () => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    ...
    if (!reducerState.inputCustomerFetching) {
        ...
        util.call<Models.Customer, void>(
            util.urlCustomerActivityExclude.callGetCustomer(state, reducerState, [{tag: Enums.CachePolicy.Default}]),
            ...
            Converters.RESPONSE_CUSTOMER_ACTIVITY_EXCLUDE_CALL_GET_CUSTOMER_TO_CUSTOMER,
 ...
 */
export const callPostCustomerActivityExcludeCreate = () => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    let state = getState()
    let reducerState = state.user.mrmkmcibCRM.reducerCustomerActivityExclude
    const teamMemberList = 
    (util.getCustomerActivityIncludeExecutor(state.user.mrmkmcibCRM.reducerCustomer.currentCustomerManaged)) ? 
    ...
    if (!reducerState.activityExcludeCreateFetching) {
        ...
        util.call<boolean, Models.CustomerActivityExcludeCreateRequest>(
            util.urlCustomerActivityExclude.callPostCustomerActivityExcludeCreate(state, reducerState, [{tag: Enums.CachePolicy.Disabled}]),
            ...
            Converters.REQUEST_CUSTOMER_ACTIVITY_EXCLUDE_CALL_POST_CUSTOMER_ACTIVITY_EXCLUDE_CREATE_FROM_CUSTOMER_ACTIVITY_EXCLUDE_CREATE_REQUEST,
            {
