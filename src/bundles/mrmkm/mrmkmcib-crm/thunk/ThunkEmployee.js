...
import { EnumsApp, performModalEmailShow, performAppOpen, performModalUserProfileHide, navigateToEmailScreen as navigateToEmailScreenModal } from "mrmkmcib-app";
import { EnumsScheduler, performPersonOpen, performReturnToActivityScreen } from "mrmkmcib-scheduler";
...
export const performSendEmail = () => (dispatch, getState) => {
    ...
    dispatch(performModalEmailShow({ data: [util.employeeToPerson(reducerState.currentEmployee)] }, '', '', false, false, ''));
...
export const performSchedulerOpen = () => (dispatch, getState) => {
    ...
    OWAService.checkCertificate()
        ...
        if (reducerState.isExtendedMode &&
            ...
            dispatch(thunkAppScheduler.performSchedulerOpen(null, Object.assign({}, defaultValues.Agent, { id: state.user.mrmkmcibCRM.reducerAgent.currentAgent.id, firstName: null, lastName: null, middleName: null, fullName: null, email: null, modId: null, gender: Enums.GenderList.None, birthday: null, isPrincipal: false, isBlocked: false, phoneList: { data: [] }, customerPositionList: { data: [] }, comment: '', memberList: { data: [] }, occasionList: { data: [] }, noteList: { data: [] } }), null, null, EnumsScheduler.SchedulerMode.Employee));
 ...
 */
export const performPopoverPersonListShow = () => (dispatch) => {
    dispatch(actionsEmployee.performPopoverPersonListShow());
};
/*
 * Переход из моего профиля в адресную книгу
 */
export const performNavigateToAddressBookEmployee = (employeeId) => (dispatch, getState) => {
    dispatch(performModalUserProfileHide());
 ...
 */
export const performPopoverPersonListHide = () => (dispatch) => {
...
export const performPersonSelect = (person) => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerEmployee;
    ...
    setTimeout(() => {
        ...
        dispatch(thunkAppScheduler.performSchedulerOpen(null, Object.assign({}, defaultValues.Agent, { id: reducerState.currentEmployee.id, firstName: reducerState.currentEmployee.firstName, lastName: reducerState.currentEmployee.lastName, middleName: reducerState.currentEmployee.middleName, fullName: reducerState.currentEmployee.fullName, email: person.email, modId: null, gender: Enums.GenderList.None, birthday: null, isPrincipal: false, isBlocked: false, phoneList: { data: [] }, customerPositionList: { data: [] }, comment: '', memberList: { data: [] }, occasionList: { data: [] }, noteList: { data: [] } }), null, reducerState.currentEmployee, EnumsScheduler.SchedulerMode.Employee));
    }, 100);
};
export const performFindPeopleSuccess = (data) => (dispatch, getState) => {
    ...
    if (reducerState.foundPeopleList.data.length > 1) {
        ...
        dispatch(performPersonSelect(Object.assign({}, ModelState.defaultValues.Person, { email: email })));
        dispatch(actionsEmployee.performSchedulerOpen());
    }
    else {
...
export const performFindPeopleFailure = (error) => (dispatch) => {
 ...
 */
export const navigateToEmployeeScreen = (employeeId, isExtendedMode, mode, historyAction = Enums.EmployeeHistoryActions.Push) => (dispatch) => {
 ...
 */
export const navigateToCustomerListScreen = () => (dispatch) => {
...
export const performRefreshSuccess = (data) => (dispatch) => {
    dispatch(actionsEmployee.performRefreshSuccess(data));
};
export const performRefreshFailure = (error) => (dispatch) => {
 ...
 */
export const callGetEmployeeHead = (head) => (dispatch, getState) => {
    const state = getState();
    const reducerState = state.user.mrmkmcibCRM.reducerEmployee;
    ...
    if (!reducerState.currentEmployeeHeadFetching) {
        ...
        util.call(util.urlEmployee.callGetEmployeeHead(state, head, [{ tag: Enums.CachePolicy.Default }, { tag: util.getEmployeeRequestRefreshString(Enums.EmployeeRequestRefresh.EmployeeRequestRefreshEnabled) }]), (response) => {
        ...
        }, Converters.RESPONSE_EMPLOYEE_REFRESH_CALL_GET_EMPLOYEE_TO_EMPLOYEE, 'GET');
    }
};
/*
 * Thunk dispatched by "Employee" screen. Fetch currentEmployee with current Id.
 */
export const refresh_callGetEmployee = () => (dispatch, getState) => {
    let state = getState();
    dispatch(thunkAppCRM.swapContext(state.user.mrmkmcibApp.reducerAuthorization.currentUser, state.user.mrmkmcibApp.reducerAppMRMKMCIB.classifierDictionary, state.user.mrmkmcibScheduler.reducerAppScheduler.currentExchangePerson, state.user.mrmkmcibApp.reducerAppMRMKMCIB.appBuildVersion, state.user.mrmkmcibApp.reducerAppMRMKMCIB.appServerUrl, state.user.mrmkmcibApp.reducerAppMRMKMCIB.appServerPath)); // для прокидывания урлов из cib-app в cib-crm
    let reducerState = state.user.mrmkmcibCRM.reducerEmployee;
    ...
    if (!reducerState.currentEmployeeFetching) {
        ...
        util.call(util.urlEmployee.refresh_callGetEmployee(getState(), reducerState, [{ tag: Enums.CachePolicy.Default }, { tag: util.getEmployeeRequestRefreshString(Enums.EmployeeRequestRefresh.EmployeeRequestRefreshEnabled) }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerEmployee;
            if (reducerState.currentEmployeeId != operationId)
                return;
 ...
 */
export const callGetCustomerList = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerEmployee;
    ...
    if (!reducerState.customerListFetching) {
        ...
        util.call(util.urlEmployee.callGetCustomerList(state, reducerState, [{ tag: Enums.CachePolicy.Default }, { tag: util.getEmployeeRequestRefreshString(Enums.EmployeeRequestRefresh.EmployeeRequestRefreshEnabled) }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerEmployee;
            if (reducerState.currentEmployeeId != operationId)
                return;
        ...
        }, Converters.RESPONSE_EMPLOYEE_CALL_GET_CUSTOMER_LIST_TO_CUSTOMER_LIST, 'GET');
    }
};
/*
 * Thunk dispatched by "Employee" screen. Fetch currentEmployee subordinate list.
 */
export const callGetSubordinateList = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerEmployee;
    ...
    if (!reducerState.subordinateListFetching) {
        ...
        util.call(util.urlEmployee.callGetSubordinateList(state, reducerState, [{ tag: Enums.CachePolicy.Default }, { tag: util.getEmployeeRequestRefreshString(Enums.EmployeeRequestRefresh.EmployeeRequestRefreshEnabled) }]), (response) => {
            state = getState();
            reducerState = state.user.mrmkmcibCRM.reducerEmployee;
            if (reducerState.currentEmployeeId != operationId)
                return;
        ...
        }, Converters.RESPONSE_EMPLOYEE_CALL_GET_SUBORDINATE_LIST_TO_MEMBER_LIST, 'GET');
    }
};
/*
 ...
 * Thunk dispatched by "Employee" screen.
 */
export const navigateBackEmployee = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerEmployee;
    let lastEmployee = reducerState.employeeNavigationHistory[reducerState.employeeNavigationHistory.length - 1];
    if (lastEmployee) {
 ...
 */
export const performPopoverSubordinateShow = (subordinate) => (dispatch) => {
    dispatch(actionsEmployee.performPopoverSubordinateShow(subordinate));
};
/*
 * Thunk dispatched by "Employee" screen. Thunk chain used to hide popover.
 */
export const performPopoverSubordinateHide = () => (dispatch) => {
...
export const navigateEmployeeScreenBack = () => (dispatch, getState) => {
    let state = getState();
    let reducerState = state.user.mrmkmcibCRM.reducerEmployee;
    ...
    switch (reducerState.currentMode) {
        ...
        case Enums.EmployeeMode.Employee: {
            ...
            if (reducerState.employeeNavigationHistory.length === 1) {
                dispatch(thunkEmployee.navigateToEmployeeScreen(reducerState.employeeNavigationHistory[0].id, true, mode, Enums.EmployeeHistoryActions.Pop));
                dispatch(thunkEmployee.navigateBack());
            }
            else {
 ...
 */
export const navigatePopoverBack = () => (dispatch) => {
 ...
 */
export const navigateToPersonContainerPopover = (person) => (dispatch) => {
 ...
 */
export const navigateToEmailScreen = () => (dispatch, getState) => {
    ...
    OWAService.checkCertificate()
        ...
        if (state.user.mrmkmcibApp.reducerProfile.isVisibleModalUserProfile) {
            dispatch(performModalEmailShow({ data: [util.employeeToPerson(reducerEmployee.currentEmployee)] }, '', '', false, false, ''));
...
export const performCancelRefreshError = () => (dispatch, getState) => {
    dispatch(actionsEmployee.performCancelRefreshError());
};
export const performCancelSubordinateListError = () => (dispatch, getState) => {
    dispatch(actionsEmployee.performCancelSubordinateListError());
};
export const performCancelCustomerListError = () => (dispatch, getState) => {
    dispatch(actionsEmployee.performCancelCustomerListError());
};
export const performCertificateAlertPopoverHide = () => (dispatch, getState) => {
    dispatch(actionsEmployee.performCertificateAlertPopoverHide());
};
export const performCertificateAlertPopoverShow = () => (dispatch, getState) => {
    dispatch(actionsEmployee.performCertificateAlertPopoverShow());
};
//# sourceMappingURL=ThunkEmployee.js.map