...
export const QLIK_CERTIFICATE_VERIFICATION = false;
const host = (state) => (`${state.user.mrmkmcibDashboard.reducerAppDashboard.appServerUrl || state.user.mrmkmcibApp.reducerAppMRMKMCIB.appServerUrl}`);
const url = (state) => (`${host(state)}/${state.user.mrmkmcibDashboard.reducerAppDashboard.appServerPath || state.user.mrmkmcibApp.reducerAppMRMKMCIB.appServerPath}`);
export const getUrlForRole = (role, supParams) => {
    let url = '';
    if (role != null) {
        switch (role) {
            case 0:
...
export const urlAppDashboard = {
    callQlikAvailabilityCheck: (state, reducerState, tagList) => {
        return {
    ...
    sendFile: (state, reducerState, tagList) => {
        return {
...
export const getFileFormatString = (key) => {
    switch (key) {
...
export const stringToFileFormat = (value) => {
    switch (value) {
        case '0': {
...
export const call = (url, success, failure, converter, method = 'GET', headers = {}, requestConverter = () => { return null; }, requestData = null) => {
...
export const ping = (url, success, failure, method = 'GET', headers = {}) => {
    //log('PING ' + method + ' ' + url)
 ...
 * @param {any} event
 */
export const convertQlikMessage = (event) => {
    let res = ModelState.defaultValues.QlikMessage;
    if (!(event && event.nativeEvent && event.nativeEvent.data)) {
        return res;
    }
    let mess = JSON.parse(event.nativeEvent.data);
    ...
    //     }
    // }
    let payload = Object.assign({}, res.payload, (mess.payload || {}));
    if (payload.clientId) {
...
export const qlikErrorMessages = {
    QlikSenseUnavailable: 'АС «Визуализация отчётности» недоступна. Пожалуйста, обратитесь к администратору',
    QlikSenseServerError: 'Произошла ошибка  АС «Визуализация отчётности» Пожалуйста, обратитесь к администратору',
    QlikSenseMashupError: 'Данные аналитики для пользователя отсутствуют или не выделена лицензия для работы с аналитикой. Обратитесь к администратору',
...
export const recoverSearchHistoryListFromStorage = () => {
...
export const persistSearchHistoryListToStorage = (list) => {
...
export const recoverPeopleHistoryListFromStorage = () => {
...
export const persistPeopleHistoryListToStorage = (list) => {
    return Storage.setItem('dashboardApp', 'personHistoryList', list.data);
};
export const chooseURL = (props) => {
...
export const getLoginLDAP = (login) => {
    return login.endsWith('-ext') ? login : `${login}-ext`;
};
export const urlCustomerDashboardTail = (baseUri, customerManagedId, kpiId, isHolding, kpiForHolding) => {
...
export const personListAdd = (personList, person) => {
    const checkUniqEmail = (array, email) => {
...
export const personListRemove = (personList, person) => {
...
export const searchHistoryListAppend = (searchHistoryList, customer) => {
...
export const getPersonName = (item) => {
...
export const personHistoryListAppend = (currentList, appendList) => {
 ...
 */
export const getCookieString = (data) => {
