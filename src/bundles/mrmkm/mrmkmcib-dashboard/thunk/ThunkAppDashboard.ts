...
import {EnumsScheduler, Models as ModelsScheduler} from "mrmkmcib-scheduler"
...
import * as thunkAppDashboard from './ThunkAppDashboard'
...
export const performApplicationInit = () => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    ...
    dispatch (thunkAppDashboard.swapContext (
 ...
 */
export const callQlikAuth = () => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    let state = getState()
    let reducerState = state.user.mrmkmcibDashboard.reducerAppDashboard
    ...
    Cookie.get((cookies: Array<CookieData>) => {
        ...
        const data:  Array<CookieData> | null = getQlikCookieData(cookies)
        ...
        fetch(qlikAuthUrl).then((response: any) => {
 ...
 */
export const swapContext = (user: ModelsApp.Employee, appBuildVersion: string, appServerUrl: string, appServerPath: string) => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    let state = getState()
    let reducerState = state.user.mrmkmcibDashboard.reducerAppDashboard

    dispatch (actionsAppDashboard.swapContext (
        user,
        appBuildVersion,
        appServerUrl,
        appServerPath,
 ...
 */
export const performInputSearchCustomerManaged = (value: boolean) => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    let state = getState()
    let reducerState = state.user.mrmkmcibDashboard.reducerAppDashboard
...
export const search_callGetCustomerSearchList = () => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    ...
    if (!reducerState.customerSearchListFetching) {
        ...
        util.call<ModelsCrm.CustomerList, void>(
            util.urlAppDashboard.search_callGetCustomerSearchList(state, reducerState, [{tag: Enums.CachePolicy.Default}]),
            ...
            Converters.RESPONSE_APP_DASHBOARD_SEARCH_CALL_GET_CUSTOMER_SEARCH_LIST_TO_CUSTOMER_LIST,
...
export const searchAppend_callGetCustomerSearchListPage = () => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    ...
    if (!reducerState.customerSearchListPageFetching) {
        ...
        util.call<ModelsCrm.CustomerListPage, void>(
            util.urlAppDashboard.searchAppend_callGetCustomerSearchListPage(state, reducerState, [{tag: Enums.CachePolicy.Default}]),
            ...
            Converters.RESPONSE_APP_DASHBOARD_SEARCH_APPEND_CALL_GET_CUSTOMER_SEARCH_LIST_PAGE_TO_CUSTOMER_LIST_PAGE,
...
export const performQlikEvent = (message: Models.QlikMessage ) => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    ...
    const navigateBackData: ModelsApp.NavigateToEntity = {...defaultValues.NavigateToEntity ,navigationType: EnumsApp.NavigationType.AppDashboard, qlikUrl: message.payload.currentUrl}
    switch (message.type) {
        ...
        case Enums.QlikEventType.OpenDealCard: {
            ...
            dispatch(performNavigateToEntity(
                // {navigationType: EnumsApp.NavigationType.CustomerDeal, customerId: '1-15000Z', dealId: '55', dealMode: EnumsCrm.DealMode.NavigationBack},
                {
        ...
        case Enums.QlikEventType.OpenClientCardActivity: {
            dispatch(performNavigateToEntity({
                ...
                isPrimary: false,
                customerMode: EnumsCrm.CustomerMode.NavigationBack,
                activityContextMode: EnumsScheduler.ActivityContextMode.Notice,
            },
            navigateBackData
            )) // '1-CHCFSE9' 100 KM
            break
 ...
 */
export const performCustomerSelect = (customer: ModelsCrm.Customer,) => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    let state = getState()
    let reducerState = state.user.mrmkmcibDashboard.reducerAppDashboard
 ...
 */
export const performCustomerOpen = (customer: ModelsCrm.Customer) => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    let state = getState()
    let reducerState = state.user.mrmkmcibDashboard.reducerAppDashboard
    ...
    dispatch(performNavigateToEntity(
        {
            ...
            customerMode: EnumsCrm.CustomerMode.NavigationBack,
            qlikUrl: reducerState.currentQlikUrl,
        },
        {
            ...defaultValues.NavigateToEntity,
            navigationType: EnumsApp.NavigationType.AppDashboard,
            qlikUrl: reducerState.currentQlikUrl
        },
    ))
 ...
 */
export const performSearchHistoryListClear = () => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    let state = getState()
    let reducerState = state.user.mrmkmcibDashboard.reducerAppDashboard
 ...
 */
export const persistSearchHistoryList = () => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    let state = getState()
    let reducerState = state.user.mrmkmcibDashboard.reducerAppDashboard
 ...
 */
export const recoverSearchHistoryList = () => (dispatch: Function, getState: () => ModelState.IRootState): void => {
...
export const recoverSearchHistoryListSuccess = (data: Models.SearchHistoryList) => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    let state = getState()
    let reducerState = state.user.mrmkmcibDashboard.reducerAppDashboard
...
export const recoverSearchHistoryListFailure = (error: Error) => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    let state = getState()
    let reducerState = state.user.mrmkmcibDashboard.reducerAppDashboard
...
export const performUpdateSupParameters = () => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    ...
    let reducerState = state.user.mrmkmcibDashboard.reducerAppDashboard
        ...
        Settings
            ...
            .catch(
                (error) => {
                    dispatch(actionsAppDashboard.performUpdateSupParametersFailure({
                        ...defaultValues.Error
                    }))
                    // Dispatch thunk "callQlikAvailabilityCheck" synchronously.
            })
...
export const performFindPeople = () => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    ...
    if (!reducerState.foundPersonListInProgress) {
        ...
        OWAService.findPeople(reducerState.inputSharePopoverSearch)
            ...
            .catch((error) => {
                dispatch(thunkAppDashboard.performFindPeopleFailure({
                    ...defaultValues.Error
                }))
            })
 ...
 */
export const setCurrentAppDashboardQlikUrl = (url: string | null) => (dispatch: Function, getState: () => ModelState.IRootState): void => {
    dispatch(actionsAppDashboard.setCurrentAppDashboardQlikUrl(url));
}

declare var __DEV__: boolean

const getQlikCookieData = (cookies: Array<CookieData>): Array<CookieData>  | null => {
    if (Array.isArray(cookies) && cookies.length) {
        return cookies.filter((cookie : CookieData) =>
        cookie.Name === 'X-Qlik-Session' ||
